...
def global_K_uh_matrix(mesh_points, mesh_elements):
    """Computes the coefficient matrix of h in the weak form of the
    x-momentum equation
    """
    ...
    for i in me_el:
        ...
        m = np.matrix([[y_23, y_31, y_12],
                       [y_23, y_31, y_12],
                       [y_23, y_31, y_12]
                      ]
                     )

        #K_uh matrix of element
        K_uh_e = (g/6.0)*(m)
        ...
        for n, index in enumerate(i):
            ...
            a = K_uh_e[n].item(n)
            row[index] += a
            row[oth_el[0]] += K_uh_e[n].item(list(i).index(oth_el[0]))
            row[oth_el[1]] += K_uh_e[n].item(list(i).index(oth_el[1]))

    return np.matrix(global_matrix)


def global_K_vh_matrix(mesh_points, mesh_elements):
    """Computes the coefficient matrix of v in the weak form of the
    y-momentum equation
    """
    #Acceleration due to gravity
    ...
    for i in me_el:
        ...
        m = np.matrix([[x_32, x_13, x_21],
                       [x_32, x_13, x_21],
                       [x_32, x_13, x_21]
                      ]
                     )

        #K_vh matrix of element
        K_vh_e = (g/6.0)*(m)
